...
An attacker can start from Stage 1 and get straignt into Stage 4 eg default admin credentials on an publicly exposed admin page. It can be non-linear & recursive in nature. Lateral movement in a network is a good example of recursiveness, it is related to staying within the network. Some would use the notion of initial foothold vs gaining a stronghold within a target network.
